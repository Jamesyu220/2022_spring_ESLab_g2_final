...
void e_fun(void const *name);
int main(){
    data_init();
    const int a1 = 5;
    
    // e_thread.start(callback(e_fun, (void *)&a1));
    _event_queue.call_every(1s, Mysensor);
    e_thread.start(mbed::callback(&_event_queue, &events::EventQueue::dispatch_forever));
    // e_thread.join();

    // EventQueue queue;
    // queue.call_every(1s, Mysensor);
...
//             acc_state_counter_z -= 1;
//         }
//         // cout << pDataXYZ[0] << " " << pDataXYZ[1] << " " << pDataXYZ[2] << endl;
//         if(acc_state_counter_y < 0 || !(acc_output &(~Y_MASK)) ){
//             if(pDataXYZ[1] > 200 && (gyro_output & (GYRO_Z_CCLK|GYRO_Z_CLK))){
//                 // acc_output = 1;
//                 acc_output |= ACC_Y_B;
